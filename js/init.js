(function ($) {
    $(function () {
        $("#submit-mail").click(function () {
            emailjs.send("default_service","temp",{name: $('#icon_prefix').val(), mail: $('#icon_email').val(), message: $('#text').val()});
        ...
        var minRatio = 0.85;
        var minWidth = width * minRatio;
        var minHeight = height * minRatio;
        var ratio = width / height;
        $(window).on("scroll", function () {
            var scrollTop = $(document).scrollTop();
            var w = ($(document).height() - scrollTop) * 0.019;
            var newWidth = w * ratio;
            var newHeight = w;
            if (newHeight > minHeight && newWidth > minWidth) {
                $("#logo").width(newWidth).height(newHeight)
            } else {
                $("#logo").width(minWidth).height(minHeight)
            }
        });
        ...
        function animateHeadline($headlines) {
            ...
            $headlines.each(function () {
                ...
                if (headline.hasClass('loading-bar')) {
                    ...
                    var words = headline.find('.cd-words-wrapper b b'),
                        width = 0;
                    words.each(function () {
